...
const SASAAnalysis = () => {
  ...
  const [sasaData, setSasaData] = useState<any[]>([]);
  ...
  const generateRandomSASAData = (length: number, variability: number) => {
    const data = [];
    let value = 10 + Math.random() * 5; // Starting with a reasonable SASA value
    
    for (let i = 0; i < length; i++) {
      const change = (Math.random() - 0.5) * variability;
      value = Math.max(5, value + change); // SASA can't be negative, keep it reasonable
      
      if (i > length / 3) {
        value = value * 0.99 + (10 + Math.random() * 5) * 0.01; // Slight stabilization
      }
  ...
  const handleRunAnalysis = async () => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          if (file.type === 'text/plain' || file.name.endsWith('.xvg') || file.name.endsWith('.txt') || file.name.endsWith('.dat')) {
            ...
            const { x, y } = parseXVGContent(fileContent);
            seriesData = x.map((xVal, idx) => [xVal, y[idx]]);
          } else {
            seriesData = generateRandomSASAData(100, 0.5);
          }
        } catch (error) {
      ...
      setSasaData(allSeriesData);
      
      const distributionSeriesData = allSeriesData.map(series => {
        const sasaValues = series.data.map((point: number[]) => point[1]);
        const distribution = calculateDistribution(sasaValues);
      ...
      toast({
        ...
        description: "SASA analysis has been successfully processed.",
      });
    } catch (error) {
  ...
  const downloadPlot = (plotId: string) => {
    ...
    if (plotElement) {
      import('plotly.js-dist').then(Plotly => {
        const chartData = plotId === 'sasa-plot' 
          ? { 
              data: sasaData.map(series => ({
                type: 'scatter',
                mode: 'lines',
  ...
  useEffect(() => {
    if (sasaData.length > 0 && seriesColors.length > 0) {
      const updatedSasaData = sasaData.map((series, index) => ({
        ...series,
        color: seriesColors[index % seriesColors.length]
      }));
      setSasaData(updatedSasaData);
    ...
    <div className="min-h-screen pt-24 pb-20">
      <div className="container px-4 mx-auto">
        ...
        >
          ...
          <h1 className="text-3xl font-bold mb-2">Solvent Accessible Surface Area Analysis</h1>
          <p className="text-muted-foreground mb-8">
            Analyze the surface area of a protein that is accessible to solvent molecules over time.
          </p>
        </motion.div>
              ...
              >
                <Tabs defaultValue="lineChart">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <TabsList>
                      <TabsTrigger value="lineChart">SASA Curve</TabsTrigger>
                  ...
                  <TabsContent value="lineChart" className="space-y-4">
                    ...
                    <div className="flex justify-end">
                      ...
                      <Button onClick={() => downloadPlot('sasa-plot')}>
            ...
            >
              ...
              <Tabs defaultValue="lineChart">
                <TabsList className="mb-4 w-full">
                  <TabsTrigger id="lineChart-tab" value="lineChart" className="flex-1">SASA Curve</TabsTrigger>
