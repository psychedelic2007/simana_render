...
const RMSFAnalysis = () => {
  ...
  const [rmsfData, setRmsfData] = useState<any[]>([]);
  ...
  // Generate random RMSF data for demonstration
  const generateRandomRMSFData = (length: number, variability: number) => {
    const data = [];
    
    // For RMSF, each residue has its own fluctuation value
    for (let i = 1; i <= length; i++) {
      // RMSF tends to be higher in loop regions and lower in structured regions
      // This simulates some structured regions with periodicity
      const structureFactor = Math.sin(i / 10) * 0.1 + 0.15;
      const randomFactor = Math.random() * variability;
      const value = Math.max(0, structureFactor + randomFactor);
      
      data.push([i, value]); // Residue number, RMSF in nm
    }
  ...
  const handleRunAnalysis = async () => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          if (file.type === 'text/plain' || file.name.endsWith('.xvg') || file.name.endsWith('.txt') || file.name.endsWith('.dat')) {
            ...
            seriesData = parsedData.x.map((x, idx) => [x, parsedData.y[idx]]);
          } else {
            // If we can't parse, generate random data
            seriesData = generateRandomRMSFData(200, 0.1);
          }
        } catch (error) {
      ...
      setRmsfData(allSeriesData);
      
      // Calculate distribution data
      const distributionSeriesData = allSeriesData.map(series => {
        const rmsfValues = series.data.map((point: number[]) => point[1]);
        const distribution = calculateDistribution(rmsfValues);
      ...
      toast.success("Analysis Complete", {
        description: "RMSF analysis has been successfully processed.",
      });
    } catch (error) {
  ...
  useEffect(() => {
    if (rmsfData.length > 0 && seriesColors.length > 0) {
      const updatedRmsfData = rmsfData.map((series, index) => ({
        ...series,
        color: seriesColors[index % seriesColors.length]
      }));
      setRmsfData(updatedRmsfData);
    ...
    <div className="min-h-screen pt-24 pb-20">
      <div className="container px-4 mx-auto">
        ...
        >
          ...
          <h1 className="text-3xl font-bold mb-2">Root Mean Square Fluctuation Analysis</h1>
          <p className="text-muted-foreground mb-8">
            Analyze the structural fluctuations of residues or atoms in a protein over time.
          </p>
        </motion.div>
              ...
              >
                <Tabs defaultValue="lineChart">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <TabsList>
                      <TabsTrigger value="lineChart">RMSF Profile</TabsTrigger>
                  ...
                  <TabsContent value="lineChart" className="space-y-4">
                    ...
                    <div className="flex justify-end">
                      ...
                      <Button onClick={() => downloadPlot('rmsf-plot')}>
            ...
            >
              ...
              <Tabs defaultValue="lineChart">
                <TabsList className="mb-4 w-full">
                  <TabsTrigger id="lineChart-tab" value="lineChart" className="flex-1">RMSF Profile</TabsTrigger>
